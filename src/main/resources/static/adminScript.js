async function getAdminInfo() {
    ...
    getAdmin(user)
    getAdminNavBar({email, roles})

}

function getAdminNavBar({email, roles}) {
    let rolesNavBar = ''
    roles.forEach(role => {
        rolesNavBar += role.name.replace('ROLE_', '') + " "
    })
    document.getElementById('headerEmail').innerHTML = email
...
function getUsersTable() {
    ...
    userTable.then(users => {
        ...
        for (let user of users) {
            const {id, username, firstName, lastName, age, email, roles} = user;
            let roleNames = roles.map(role => role.name.replace('ROLE_', ''));
            res += `<tr>
            <td>${id}</td>
            <td>${username}</td>
            <td>${firstName}</td>
            ...
            <td>
                ...
                onclick="editModal(${id})">Edit</button>
            </td>
        
            <td>
                <button class="btn btn-danger" data-toggle="modal"
                data-target="#deleteModal" 
                onclick="deleteModal(${id})">Delete</button>
            </td>
        </tr>`
        }
